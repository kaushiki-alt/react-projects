...
const CartItem = ({ cartItem }) => {
    ...
    const { title, company, amount, price, productColor, image, cartID } = cartItem
    return (
        <article key={cartID} className='mb-12 flex-wrap flex gap-4 flex-col sm:flex-row border-b border-base-300 pb-6 last:border-b-0'>
            <img
                src={image}
                alt={title}
                className='size-24 sm:size-32 rounded-lg object-cover' />
