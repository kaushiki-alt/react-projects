...
export const mostUsedLang = (repositories: Repository[]): { language: string, count: number }[] => {
    if (repositories.length === 0) {
        return [];
    }
    ...
    if (Object.keys(languageMap).length === 0) {
       ...
       return (
        Object.entries(languageMap).sort(([, a], [, b]) => b - a).slice(0,5).map(([language, count]) => ({ language, count }))
  );
}
